...
async function getExchangeInfo(symbol, testnet) {
  const result = await publicRequest("/api/v3/exchangeInfo", symbol ? { symbol } : {}, testnet);
  return {
...
async function getTickerPrices(testnet) {
  const result = await publicRequest("/api/v3/ticker/price", {}, testnet);
...
async function getTickerPrice(symbol, testnet) {
  const [price, stats] = await Promise.all([
    publicRequest("/api/v3/ticker/price", { symbol }, testnet),
    publicRequest("/api/v3/ticker/24hr", { symbol }, testnet),
  ]);
...
async function getTicker24hr(symbolsOrTestnet = false, maybeTestnet = false) {
  ...
  const result = await publicRequest("/api/v3/ticker/24hr", {}, testnet);
...
async function getCandles(symbol, interval = "15m", limit = 48, testnet = false) {
  const result = await publicRequest("/api/v3/klines", {
    symbol,
