...
function getFuturesBaseUrl(testnet) {
  if (testnet) {
    return process.env.BINANCE_FUTURES_TESTNET_URL || "https://testnet.binancefuture.com";
  }
  return process.env.BINANCE_FUTURES_BASE_URL || "https://fapi.binance.com";
}
...
function normalizeFuturesOrder(order) {
  ...
  return {
    ...
    timeInForce: String(order.timeInForce || "").toUpperCase(),
    transactTime: Number(order.updateTime || order.time || Date.now()),
  };
}

function normalizeFuturesBalance(asset) {
  return {
    asset: String(asset.asset || "").toUpperCase(),
    walletBalance: String(asset.walletBalance ?? asset.balance ?? "0"),
    availableBalance: String(asset.availableBalance ?? "0"),
    unrealizedProfit: String(asset.unrealizedProfit ?? "0"),
    marginBalance: String(asset.marginBalance ?? "0"),
    maxWithdrawAmount: String(asset.maxWithdrawAmount ?? "0"),
  };
}

function normalizeFuturesPosition(position) {
  ...
  return {
    ...
    leverage: String(position.leverage || "0"),
    marginType: String(position.marginType || "").toUpperCase(),
    isolatedMargin: String(position.isolatedMargin || "0"),
    notional: String(position.notional || "0"),
    positionSide: String(position.positionSide || "BOTH").toUpperCase(),
    updateTime: Number(position.updateTime || Date.now()),
  };
}
...
async function publicFuturesRequest(path, params = {}, testnet = false) {
  const query = buildQuery(params);
  const url = `${getFuturesBaseUrl(testnet)}${path}${query ? `?${query}` : ""}`;
  const response = await fetch(url, {
    signal: AbortSignal.timeout(15000),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(normalizeErrorMessage(payload, `Unable to reach Binance futures. HTTP ${response.status}.`));
  }
  return payload;
}
...
async function getFuturesServerTime(testnet) {
  return publicFuturesRequest("/fapi/v1/time", {}, testnet);
}
...
async function syncFuturesServerTimeOffset(testnet) {
  const startedAt = Date.now();
  const payload = await getFuturesServerTime(testnet);
  const finishedAt = Date.now();
  const midpoint = startedAt + Math.round((finishedAt - startedAt) / 2);
  const offset = Number(payload.serverTime || 0) - midpoint;
  const key = testnet ? "futuresTestnet" : "futuresMainnet";
  serverTimeOffsetMs[key] = offset;
  serverTimeSynced[key] = true;
  return offset;
}
...
async function signedFuturesRequest(account, path, method = "GET", params = {}, allowRetry = true) {
  const key = account.testnet ? "futuresTestnet" : "futuresMainnet";
  if (!serverTimeSynced[key]) {
  ...
  const signature = crypto.createHmac("sha256", secret).update(queryString).digest("hex");
  const finalQuery = `${queryString}&signature=${signature}`;
  const url = `${getFuturesBaseUrl(account.testnet)}${path}?${finalQuery}`;
  const response = await fetch(url, {
    method,
    signal: AbortSignal.timeout(15000),
    headers: {
...
async function cancelFuturesOrder(account, symbol, orderId) {
  const result = await signedFuturesRequest(account, "/fapi/v1/order", "DELETE", {
    symbol,
    orderId: String(orderId),
  });
...
async function closeFuturesPosition(account, symbol, positionSide = "BOTH", quantity) {
  ...
  if (!target) {
    throw new Error(`No open Binance futures position was found for ${symbol}.`);
  }

  const currentQuantity = Math.abs(Number(target.positionAmt || 0));
  const closeQuantity = Number(quantity || 0) > 0 ? Math.min(Number(quantity), currentQuantity) : currentQuantity;
  if (!closeQuantity) {
    throw new Error(`No futures quantity is available to close for ${symbol}.`);
  }

  const result = await signedFuturesRequest(account, "/fapi/v1/order", "POST", {
    symbol,
    side: Number(target.positionAmt || 0) > 0 ? "SELL" : "BUY",
    type: "MARKET",
    quantity: String(closeQuantity),
    reduceOnly: normalizedPositionSide === "BOTH" ? "true" : undefined,
    positionSide: normalizedPositionSide === "BOTH" ? undefined : normalizedPositionSide,
